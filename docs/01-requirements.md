...
| FR-107 | カテゴリ一覧 | 商品検索の絞り込み用に全カテゴリ（階層情報付き）を返す | 必須 |

FR-107 は当初の要件定義から漏れていたため追記（2026-09-02）。
カテゴリの**登録・編集・削除はフェーズ1のスコープ外**。
改廃は年数回・マーチャンダイジング担当のみのため、初期データ移行で投入し、以後は運用対応とする
（クライアント合意。オペレーターが自由に作れると分類が荒れるため、あえてAPIを設けない）。
...
| ~~O-4~~ | ~~全SKUが販売停止の商品を、管理画面の一覧でどう扱うか~~ → **回答済み（案B採用。下記詳細）** | MIN-011 着手前 | クライアント |
...
**決定（2026-09-02 クライアント回答）: 案B を採用する。**

- 理由: 「販売中に見えるのに注文が通らない商品」の調査コスト（問い合わせ1件平均25分）が
  本件刷新の動機のひとつであり、案Aはそれを放置する。案Cは商品が一覧から消えることで
  「削除されたのでは」という誤解を生み、棚卸し・マスタ確認業務でも停止中商品の可視性が必要。
- 一覧APIは「販売可能な（active な）SKUが1件以上あるか」を示すフラグを返し、
  管理画面はそれを警告表示に使う。
- 性能条件: 一覧の p95 ≤ 500ms（NFR-02）に収まること。収まらない場合は
  **警告を一覧から外し詳細画面（FR-104）のみに縮退してよい**（クライアント合意済み）。
- 詳細は [design/MIN-011-product-list.md](design/MIN-011-product-list.md)。

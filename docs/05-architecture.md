...
管理画面はAPIと別オリジンで動く（ローカル: `localhost:5173` と `localhost:8080`、
本番: CloudFront と ALB）。ブラウザからアクセスできるようCORSヘッダを返す。

| 項目 | 設定 | 理由 |
| --- | --- | --- |
| 許可オリジン | 環境変数 `ALLOWED_ORIGINS` の完全一致リスト | ワイルドカードは使わない。認証トークンと個人情報を扱うため |
| 許可ヘッダ | `Authorization`, `Content-Type`, `Idempotency-Key`, `X-Request-ID` | 漏れるとプリフライトで弾かれる |
| 公開ヘッダ | `X-Request-ID`, `Idempotent-Replay` | 明示しないとCORS越しにJSから読めない |
| プリフライトキャッシュ | 10分 | 操作のたびにOPTIONSが飛ぶ往復を減らす |

**CORS設定ミスはサーバーログに痕跡が残らない。** APIは200を返しており、
ブラウザ側が読み取りを拒否するだけなので、画面には `Failed to fetch` としか出ない。
そのため起動時ログに許可オリジンを出力し、設定の誤りは `config.Load` で起動前に弾く。
